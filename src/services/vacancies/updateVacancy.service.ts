...
const updateVacancyservice = async (id: string, { name, salary, description, vacancy_skills }: IVacancy): Promise<Vacancies> => {
  ...
  if (vacancy_skills) {
    ...
    await vacancySkillsRepository.update(vacancy.vacancy_skills.id, { ...vacancy_skills });
  }
