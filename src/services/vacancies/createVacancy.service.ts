...
const createVacancyService = async ({ name, salary, description, companyId, vacancy_skills }: IVacancy): Promise<Vacancies> => {

  const vacanciesRepository = AppDataSource.getRepository(Vacancies);

  if (!name || !salary || !description) {
    throw new AppError("All fields must be filled. (name, salary, description and company.)", 400);
  };
